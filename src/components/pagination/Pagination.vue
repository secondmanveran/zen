...
const pages = computed(() => {
  ...
  if (props.totalPages > 2) {
    ...
    if (
      ...
      props.modelValue === props.totalPages - 1
    )
      return [props.totalPages - 2, props.totalPages - 1]
...
<template>
  <ul :style="styles" :class="[className, classes.wrapper]">
    ...
    <template v-else-if="variant === 'simple'">
      ...
      <x-icon
        v-if="totalPages > 3 && modelValue > 3"
        class="mx-1"
        :icon="dotsIcon"
        :size="size" />
      <x-pagination-item
        ...
        @input="(value: Event) => $emit('update:modelValue', value)" />
      <x-icon
        v-if="totalPages > 3 && modelValue < totalPages - 2"
        class="mx-1"
        :icon="dotsIcon"
        :size="size" />
      <x-pagination-item
    ...
    <template v-else-if="variant === 'dots'">
      <li
        ...
        :class="[
          classes.dots,
          i === modelValue
            ? 'bg-[color:var(--x-pagination-bg)]'
            : 'bg-gray-100 hover:bg-gray-200'
        ]"
        @click="$emit('update:modelValue', i)"></li>
    </template>
  </ul>
</template>
