...
const props = defineProps({
  align: {
    ...
    validator: (value: string) => {
      const alignment = ['bottom', 'center', 'left', 'right', 'top']

      return alignment.includes(value)
    }
  },
  position: {
    ...
    default: 'bottom',
    validator: (value: string) => {
      const positioning = ['bottom', 'left', 'right', 'top']

      return positioning.includes(value)
    }
  },
  dismissOnClick: {
    type: Boolean,
...
const contentClasses = computed(() => {
  ...
  if (align === 'left' && ['bottom', 'top'].includes(position))
    c.push('left-0 right-auto')
  if (align === 'center' && ['bottom', 'top'].includes(position))
    c.push('left-1/2 right-auto -translate-x-1/2')
  if (align === 'right' && ['bottom', 'top'].includes(position))
    c.push('right-0 left-auto')
  if (align === 'top' && ['left', 'right'].includes(position))
    c.push('top-0 bottom-auto')
  if (align === 'center' && ['left', 'right'].includes(position))
    c.push('-translate-y-1/2 top-1/2 bottom-auto')
  if (align === 'bottom' && ['left', 'right'].includes(position))
    c.push('bottom-0')
...
<template>
  <div
    ...
    <div
      ...
      :class="[$style.popoverContent, contentClasses, classes.content]"
