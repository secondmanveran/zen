  ...
  classes: {
    ...
    list: ({ props, colors, css }: ThemeParams) => {
      ...
      if (props.variant === 'line')
        c.push('border-b border-gray-200 dark:border-gray-700')
    ...
    tracker: ({ props }: ThemeParams) => {
      ...
      if (props.variant === 'line')
        c.push(
          'h-[2px] -mt-[2px] bg-[color:var(--x-tabs-text)] dark:bg-[color:var(--x-tabs-dark-text)]'
        )

      if (props.variant === 'block')
        c.push(
          'rounded-md h-full top-0 bg-[color:var(--x-tabs-bg)] dark:bg-[color:var(--x-tabs-dark-bg)]'
        )
