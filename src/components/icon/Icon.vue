...
watchEffect(() => {
  ...
  if (injectedIcon) {
    if (typeof injectedIcon === 'string') {
      ...
      computedViewBox.value =
        injectedIcon.viewBox || injectedIcon.viewbox || props.viewBox
    }
  } else {
...
function getSVG(svgString: string) {
  ...
  const content = svgString.substring(
    svgString.indexOf('>') + 1,
    svgString.lastIndexOf('</svg>')
  )
  const attrsRaw = svgString
    .substring(svgString.indexOf('<svg') + 4, svgString.indexOf('>'))
    .trim()
    .match(/[\w-]+="[^"]*"/g)
