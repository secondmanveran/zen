...
const { errorInternal, reset, validate, setError } = useInputtable(props, {
  focus,
  emit,
  withListeners: false
})

const { styles, classes, className } = useTheme('radio', theme, props, {
  selected
})
...
<template>
  <label
    ...
    <div class="flex items-center" :class="{ 'cursor-not-allowed': disabled }">
      <input
        v-model="selected"
        :name="name"
        :required="required"
      ...
      <div
        ...
        <svg
          ...
          :class="[classes.circleIcon, $style['radio_circle']]">
          <circle cx="10" cy="10" r="10" />
        </svg>
      </div>
      <div>
        <div v-if="label" :class="classes.label" v-text="label"></div>

        <div v-if="$slots.default" :class="classes.content">
          <slot></slot>
        </div>
      </div>
    </div>
...
.radio {
  ...
  &--glow {
    box-shadow: 0 0 #000, 0 0 #000, 0 10px 15px -3px var(--x-radio-glow),
      0 4px 6px -4px var(--x-radio-glow);
  }
