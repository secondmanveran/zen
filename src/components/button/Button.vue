...
const computedColor = computed(
  () => props.color || buttonGroup.groupProps.color
)
const computedGhost = computed(
  () => props.ghost || buttonGroup.groupProps.ghost
)
const computedLight = computed(
  () => props.light || buttonGroup.groupProps.light
)
const computedOutlined = computed(
  () => props.outlined || buttonGroup.groupProps.outlined
)
const computedDisabled = computed(
  () => props.disabled || buttonGroup.groupProps.disabled
)
const computedIconLeft = computed(() => props.icon || props.iconLeft)

const attrs = useAttrs()
const htmlTag = computed(() =>
  attrs.href ? 'a' : props.to ? 'router-link' : props.tag
)
...
const { className, classes, styles } = useTheme(
  'button',
  theme,
  computedProps,
  {
...
<template>
  <component
    ...
    <x-icon
      ...
      :class="[classes.iconLeft, { invisible: loading }]" />
    ...
    <x-icon
      ...
      :size="computedSize"
      :icon="iconRight"
      :class="[classes.iconRight, { invisible: loading }]" />
  </component>
</template>
...
.button {
  ...
  &--glow {
    box-shadow: 0 0 #000, 0 0 #000, 0 10px 15px -3px var(--x-button-glow),
      0 4px 6px -4px var(--x-button-glow);
  }
  ...
  &:global(.dark) {
    ...
    &:hover {
      color: var(
        --x-button-dark-text-hover,
        var(--x-button-dark-text, var(--x-button-text))
      );
      background-color: var(
        --x-button-dark-bg-hover,
        var(--x-button-dark-bg, var(--x-button-bg))
      );
      border-color: var(
        --x-button-dark-border-hover,
        var(--x-button-dark-border, var(--x-button-border))
      );
    }
    ...
    &:active {
      ...
      background-color: var(
        --x-button-dark-bg-active,
        var(--x-button-dark-bg, var(--x-button-bg))
      );
      border-color: var(
        --x-button-dark-border-active,
        var(--x-button-dark-border, var(--x-button-border))
      );
    }
  }
