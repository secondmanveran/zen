...
export default {
  classes: {
    wrapper({ props, slots, data }: ThemeParams) {
      /*tw*/
      const classes = [
        'relative transition duration-150 focus:outline-none inline-flex items-center justify-center font-medium whitespace-nowrap overflow-hidden align-middle active:!shadow-none border appearance-none'
      ]
      ...
      if (props.size === 'xs')
        classes.push(
          slots.default
            ? `py-1 text-xs ${props.icon ? 'px-2' : 'px-3'}`
            : 'leading-none p-1'
        )
      else if (props.size === 'sm')
        classes.push(
          slots.default
            ? `py-2 text-sm ${props.icon ? 'px-3' : 'px-4'}`
            : 'leading-none p-2'
        )
      else if (props.size === 'lg')
        classes.push(
          slots.default
            ? `py-3 text-lg ${props.icon ? 'px-4' : 'px-6'}`
            : 'leading-none p-3'
        )
      else if (props.size === 'xl')
        classes.push(
          slots.default
            ? `py-4 text-xl ${props.icon ? 'px-6' : 'px-6'}`
            : 'leading-none p-4'
        )
      else
        classes.push(
          slots.default
            ? `py-2 ${props.icon ? 'px-4' : 'px-5'}`
            : 'leading-none p-2'
        )
  ...
  styles({ props, colors, css, data }: ThemeParams) {
    ...
    if (props.glow)
      vars.push(
        css.get(
          'glow',
          colors.getColorOpacity(props.color ? color[500] : gray[500], 0.5)
        )
      )
    ...
    // light
      ...
      if (!props.outlined || props.disabled)
        vars.push(css.get('border', 'transparent'))
