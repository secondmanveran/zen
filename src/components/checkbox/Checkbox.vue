...
const { errorInternal, isInsideForm, reset, validate, setError } =
  useInputtable(props, { focus, emit, withListeners: false })

const { styles, classes, className } = useTheme('checkbox', theme, props, {
  checked
})
...
.checkbox {
  &--glow {
    box-shadow: 0 0 #000, 0 0 #000, 0 10px 15px -3px var(--x-checkbox-glow),
      0 4px 6px -4px var(--x-checkbox-glow);
  }
}
</style>
