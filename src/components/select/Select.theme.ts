...
export default {
  classes: {
    ...
    box: ({ props, data }: ThemeParams) => {
      let c =
        'w-full border border-gray-300 dark:border-gray-700 pr-8 outline-transparent outline outline-2 outline-offset-[-1px] transition-all duration-150 ease-in-out rounded-md shadow-sm'

      if (!data.errorInternal && !props.disabled)
        c += ' hover:border-gray-400 dark:hover:border-gray-500'
