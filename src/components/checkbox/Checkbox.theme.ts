...
export default {
  classes: {
    ...
    box: ({ props }: ThemeParams) => {
      const classes = [
        'rounded flex justify-center items-center shrink-0 border-2 border-[color:var(--x-checkbox-border)] bg-[color:var(--x-checkbox-bg)] dark:border-[color:var(--x-checkbox-dark-border)] dark:bg-[color:var(--x-checkbox-dark-bg)]'
      ]
    ...
    label: ({ props }: ThemeParams) => {
      const classes = [
        'inline-block font-medium text-gray-800 dark:text-gray-200 pl-2'
      ]
