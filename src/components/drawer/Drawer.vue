...
const breakpoints = useBreakpoints(
  isTailwindBreakpoint
    ? breakpointsTailwind
    : ({ md: props.breakpoint || 768 } as Breakpoints)
)
const point = breakpoints.smaller(
  isTailwindBreakpoint ? props.breakpoint : 'md'
)
    ...
    <transition
      ...
      <div
        ...
        <div
          v-if="detached && value"
          ref="swipeRef"
          class="flex flex-col max-h-full"></div>
        <div
          ref="drawerRef"
          :class="[className, autoClasses, classes.wrapper]"
          :style="[autoStyles, styles]">
          <slot name="header"></slot>
          <x-scroll :scrollbar="false" vertical class="flex-1">
            <slot></slot>
          </x-scroll>
