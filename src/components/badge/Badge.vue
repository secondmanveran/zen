...
const offsetStyle = computed(() => {
  ...
  if (props.offsetX)
    style[props.align === 'left' ? 'marginLeft' : 'marginRight'] =
      props.offsetX + 'px'
  if (props.offsetY)
    style[props.position === 'top' ? 'marginTop' : 'marginBottom'] =
      props.offsetY + 'px'
...
<template>
  <component :is="tag" :style="styles" :class="[className, classes.wrapper]">
    <div class="relative inline-block">
      ...
      <div
        ...
        <div
          v-if="animated"
          class="animate-ping opacity-75 absolute top-0 left-0 rounded-full h-full w-full z-10 bg-[color:var(--x-badge-bg)]"></div>
        <div
