...
function check() {
  if (elRef.value && (elRef.value as typeof XLink).$el && props.to) {
    const active = (elRef.value as typeof XLink).$el.classList.contains(
      computedExact.value ? 'router-link-exact-active' : 'router-link-active'
    )
...
<template>
  <li
    ...
    :class="[className, { 'flex-1': tabs.state.grow }]">
    <component
      :is="to ? XLink : tag"
      ref="elRef"
      :to="to"
      ...
      :class="[
        classes.wrapper,
        {
          'text-[color:var(--x-tabs-text)] dark:text-[color:var(--x-tabs-dark-text)]':
            selected,
