...
import XInputError from '@/components/helpers/InputError.vue'
...
watch(
  () => popoverRef.value?.isOpen,
  () => {
    if (
      popoverRef.value?.isOpen &&
      (props.multiple || typeof selectedIndex.value === 'undefined')
    )
...
watch(selectedIndex, index => {
  if (typeof index !== 'undefined' && itemsRef.value)
    itemsRef.value[index].$el.scrollIntoView({
      block: 'nearest',
      inline: 'nearest'
    })
})
...
<template>
  <label
    ...
    :style="styles"
    :class="[className, classes.wrapper, { 'mb-3': isInsideForm }]">
    <p v-if="label" :class="classes.label" v-text="label"></p>
    <div class="relative">
      <x-popover
        ref="popoverRef"
        block
        ...
        <div
          :class="[
            ...
            {
              // error
              'border-red-500 dark:border-red-400 group-focus:outline-red-500':
                errorInternal,
          ...
          <template v-else>
            <div v-if="placeholder" class="text-gray-400 dark:text-gray-500">
              {{ placeholder }}
            </div>
            <div v-else>&nbsp;</div>
          </template>
        ...
        <template #content>
          <x-popover-container :class="classes.content">
            ...
            <div v-else class="px-2 text-center text-gray-400">No options</div>
          </x-popover-container>
        </template>
      </x-popover>
