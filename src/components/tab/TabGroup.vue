...
const updateTracker = useThrottleFn((value: string | number | undefined) => {
  ...
  const tabEl = tabsRef.value?.querySelector(
    `[data-value="${value}"]`
  ) as HTMLElement
  ...
  const center =
    tabEl.offsetLeft -
    (tabsRef.value.getBoundingClientRect().width -
      tabEl.getBoundingClientRect().width) /
      2
  ...
  <div>
    <div
      ...
      <x-scroll
        ...
        <ul ref="tabsRef" class="relative" :class="classes.list">
          <slot></slot>
        </ul>
        <div ref="trackerRef" :class="classes.tracker"></div>
      </x-scroll>
    </div>
