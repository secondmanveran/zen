...
const htmlTag = computed(() =>
  cItem.value.to || cItem.value.href ? XLink : 'div'
)
...
function check() {
  if (elRef.value && elRef.value.$el && (cItem.value.href || cItem.value.to)) {
    const active =
      elRef.value.$el.classList.contains(
        cItem.value.exact ? 'router-link-exact-active' : 'router-link-active'
      ) || false
...
const { styles, classes, className } = useTheme(
  'menu-item',
  theme,
  computedProps,
  { isActive }
  ...
  <component
    ...
    <span class="ml-1 shrink-0">
      ...
      <template v-else>
        ...
        <x-icon
          v-else-if="cItem.iconRight"
          :size="cItem.size"
          :icon="cItem.iconRight" />
      </template>
    </span>
  </component>
</template>
  ...
  &:global(.dark) {
    ...
    &:hover {
      ...
      background-color: var(
        --x-menu-item-dark-bg-hover,
        var(--x-menu-item-dark-bg)
      );
    }
  }
}
